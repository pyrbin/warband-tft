...
pub fn aim_projectile_straight_fallback(
    ...
    aim_projectile(target_offset, target_rel_v, proj_speed, gravity)
        .unwrap_or(target_offset.normalize_or_zero() * proj_speed)
}
...
pub fn aim_projectile(
    ...
    t.and_then(|t| calc_time_to_impact(target_offset + target_rel_v * t, proj_speed, -gravity))
        .map(|t| {
            (target_offset + target_rel_v * t - 0.5 * t * t * gravity + VERTICAL_CORRECTION * t)
                .normalize_or_zero()
                * proj_speed
        })
}
